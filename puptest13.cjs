...
(async () => {
  ...
  await page.goto('http://localhost:5251/', { waitUntil: 'domcontentloaded', timeout: 15000 });
  ...
  console.log('clicking advanced toggle...');
  const ok = await clickByText('Покупка в один тап');
  console.log('click result:', ok);
  await new Promise(r => setTimeout(r, 800));
  try {
