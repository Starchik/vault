...
(async () => {
  ...
  await page.goto('http://localhost:5261/', { waitUntil: 'domcontentloaded', timeout: 15000 });
  ...
  console.log('=== BUY TAB (with MoonPay/Transak direct links) ===');
  console.log(await page.evaluate(() => document.body.innerText));

  const links = await page.$$eval('a[target="_blank"]', els => els.map(e => e.href));
